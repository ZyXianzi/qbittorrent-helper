...
def load_config(path: Path) -> AppConfig:
    ...
    for module_name, module_value in modules_section.items():
        ...
        enabled = _expect_bool(
            module_section.get("enabled"), f"modules.{module_name}.enabled"
        )
    ...
    return AppConfig(
        ...
        logging=LoggingConfig(
            file=Path(
                _expect_str(logging_section.get("file"), "logging.file")
            ).expanduser(),
            ...
            rotate_when=_expect_str(
                logging_section.get("rotate_when"), "logging.rotate_when"
            ),
